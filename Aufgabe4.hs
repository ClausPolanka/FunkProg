...
calculateProductOf m1 m2 = [foo2 rowVector m2 | m_i <- [0..(length m1 - 1)], let rowVector = (m1 !! m_i)]

foo2 rowVector m2 = [productOf rowVector columnVector | p_i <- [0..(p - 1)], let columnVector = [x !! p_i | x <- m2]]
    where 
